...
const CartContainer = () => {
  const { cartItems, numberOfItemsAddedToCart: numberOfItemTypes, totalAmount } = useSelector((store) => store.cart);
    ...
    <section className={'text-2xl py-10'}>
      ...
      <footer>
        ...
        <div className={'grid grid-cols-10'}>
          ...
          <span className={'col-start-8 font-bold'}>{totalAmount} JPY</span>
        </div>
        <div className={'text-center'}>
          <button className={'text-red-800 border-2 border-red-700 p-4 rounded-lg hover:bg-gray-200 mt-10'}>
            Delete all
          </button>
        </div>
      </footer>
